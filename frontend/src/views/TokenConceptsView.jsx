  ...
  const renderSection4 = () => {
    ...
    return (
      <div>
        ...
        {/* How to deploy - only show at top when browser deploy is available */}
        {showDeploy && (
          <div style={{
            background: 'rgba(59, 130, 246, 0.1)',
            border: '1px solid rgba(59, 130, 246, 0.3)',
            ...
            <div style={{ color: '#93c5fd', fontWeight: 'bold', marginBottom: '0.5rem' }}>How to deploy</div>
            <p style={{ color: '#cbd5e1', fontSize: '0.9rem', margin: '0 0 0.5rem 0' }}>
              <strong>Option A (Browser):</strong> Use the button below. Requires wallet + ETH + RPC (see Connection Setup in the top bar).
            </p>
            <p style={{ color: '#cbd5e1', fontSize: '0.9rem', margin: 0 }}>
              <strong>Option B (CLI):</strong> Open a terminal in the project folder. Ensure the blockchain is running (instructor node or <code style={{ fontSize: '0.85rem' }}>npm run chain</code>). Run <code style={{ background: 'rgba(0,0,0,0.3)', padding: '0.15rem 0.4rem', borderRadius: '0.25rem', fontSize: '0.85rem' }}>node scripts/cli-labs/standalone/5-contract-builder.js</code>. Choose 1 (Build New Contract), then pick <strong>Simple Fungible Token</strong>, <strong>Simple NFT</strong>, or <strong>Event Tickets</strong> from the template list.
            </p>
          </div>
        )}
        ...
        {/* Prerequisite gate */}
        <div style={{
          background: prereqOk ? 'rgba(16, 185, 129, 0.1)' : 'rgba(251, 191, 36, 0.1)',
          border: `1px solid ${prereqOk ? '#10b981' : '#fbbf24'}`,
          borderRadius: '0.5rem',
          padding: '1rem',
          marginBottom: '1.5rem'
        }}>
          <div style={{ color: prereqOk ? '#10b981' : '#fbbf24', fontWeight: 'bold', marginBottom: '0.5rem' }}>
            {prereqOk ? 'Ready for browser deploy' : 'For browser deploy you need'}
          </div>
          <div style={{ color: '#cbd5e1', fontSize: '0.9rem' }}>
            {prereqCheck.wallet ? '✓ Wallet' : '✗ Wallet (Account Manager in Live tab)'} ·{' '}
            {prereqCheck.balance ? '✓ ETH (≥0.1)' : '✗ ETH (Get 5 ETH faucet in Live tab)'} ·{' '}
            {prereqCheck.rpc ? '✓ RPC' : '✗ RPC (Connection Setup)'}
          </div>
          {!prereqOk && (
            <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center' }}>
                <button
                  onClick={() => setUseSimulators(true)}
                  style={{
                ...
                >
                  Use Simulated Visualizers (no setup needed)
                </button>
                {onGoToLive && (
                  <button
                    onClick={onGoToLive}
                  ...
                  >
                    Go to Live tab to set up wallet
                  </button>
                )}
              </div>
              <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>
                Simulated visualizers let you explore FT and NFT transfers without a wallet. Or set up wallet + ETH + RPC in the Live tab for real deploy.
              </p>
            </div>
          )}
          {useSimulators && (
            <button onClick={() => setUseSimulators(false)} style={{ marginTop: '0.5rem', background: 'rgba(59, 130, 246, 0.2)', border: '1px solid #3b82f6', borderRadius: '0.25rem', color: '#93c5fd', padding: '0.25rem 0.5rem', cursor: 'pointer' }}>Try deploy again</button>
          )}
        </div>

        {/* Deploy section - only show when browser deploy available */}
        {showDeploy && (
          <>
        <h4 style={{ color: '#10b981', marginBottom: '0.75rem' }}>1. Deploy FT (Fungible Token)</h4>
            <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: '1rem' }}>
              Click to deploy from this page, or use the CLI (Option B above).
            </p>
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1rem' }}>
              <button onClick={handleDeployFT} disabled={ftDeploying} style={{
                padding: '0.75rem 1.5rem', background: '#10b981', color: '#fff', border: 'none', borderRadius: '0.375rem',
                fontWeight: 'bold', cursor: ftDeploying ? 'disabled' : 'pointer'
              }}>
                {ftDeploying ? 'Deploying...' : 'Deploy FT (Browser)'}
              </button>
            </div>
            {ftDeployError && <div style={{ color: '#ef4444', marginBottom: '1rem', fontSize: '0.9rem' }}>{ftDeployError}</div>}
            {ftAddress && (
              <div style={{
                background: '#0f172a',
                padding: '1.5rem',
                ...
                <h4 style={{ color: '#10b981', marginTop: 0, marginBottom: '0.5rem' }}>
                  Guided Exercise: FT in Action
                </h4>
                <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: '1rem' }}>
                  Follow the steps below to see how mint, transfer, and totalSupply work.
                </p>
                <div style={{ color: '#64748b', fontSize: '0.8rem', marginBottom: '1rem' }}>
                  FT deployed: <code style={{ fontSize: '0.85rem' }}>{ftAddress}</code>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.9rem', color: '#cbd5e1' }}>
                    <span>You</span>
                    <span style={{ textAlign: 'right', color: '#3b82f6' }}>{ftReads.myBalance ?? '—'}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.9rem', color: '#cbd5e1' }}>
                    <span>Account 1 ({ACCOUNT_1.slice(0, 10)}...)</span>
                    <span style={{ textAlign: 'right', color: '#10b981' }}>{ftReads.account1Balance ?? '—'}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.9rem', color: '#cbd5e1', marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid #334155' }}>
                    <span>Total supply</span>
                    <span style={{ textAlign: 'right', color: '#f59e0b' }}>{ftReads.totalSupply ?? '—'}</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{
                    ...
                    gap: '0.75rem',
                    padding: '0.75rem',
                    background: ftMinted ? 'rgba(16, 185, 129, 0.1)' : 'rgba(71, 85, 105, 0.2)',
                    borderRadius: '0.375rem',
                    border: `1px solid ${ftMinted ? '#10b981' : '#334155'}`
                  }}>
                    <span style={{
                      width: '28px',
                      height: '28px',
                      borderRadius: '50%',
                      background: ftMinted ? '#10b981' : '#334155',
                      color: '#fff',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#cbd5e1', fontWeight: 'bold', marginBottom: '0.25rem' }}>Mint 100 tokens to yourself</div>
                      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Creates new tokens and assigns them to your wallet.</div>
                    </div>
                    <button
                      onClick={handleMintFT}
                      disabled={!!ftTxPending || ftMinted}
                      style={{
                        padding: '0.5rem 1rem',
                        borderRadius: '0.375rem',
                        background: ftMinted ? '#334155' : '#10b981',
                        color: '#fff',
                        border: 'none',
                        cursor: ftTxPending || ftMinted ? 'not-allowed' : 'pointer',
                        fontWeight: 'bold',
                        fontSize: '0.9rem'
                      }}
                    >
                      {ftTxPending === 'mint' ? 'Minting...' : ftMinted ? 'Done' : 'Mint 100'}
                    </button>
                  </div>
                  ...
                  <div style={{
                    ...
                    gap: '0.75rem',
                    padding: '0.75rem',
                    background: ftTransferred ? 'rgba(16, 185, 129, 0.1)' : 'rgba(71, 85, 105, 0.2)',
                    borderRadius: '0.375rem',
                    border: `1px solid ${ftTransferred ? '#10b981' : '#334155'}`
                  }}>
                    <span style={{
                      width: '28px',
                      height: '28px',
                      borderRadius: '50%',
                      background: ftTransferred ? '#10b981' : '#334155',
                      color: '#fff',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#cbd5e1', fontWeight: 'bold', marginBottom: '0.25rem' }}>Transfer 30 to Account 1</div>
                      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Move tokens from your balance to {ACCOUNT_1.slice(0, 10)}...</div>
                    </div>
                    <button
                      onClick={handleTransferFT}
                      disabled={!!ftTxPending || ftTransferred || !ftMinted}
                      style={{
                        padding: '0.5rem 1rem',
                        borderRadius: '0.375rem',
                        background: ftTransferred ? '#334155' : !ftMinted ? '#475569' : '#10b981',
                        color: '#fff',
                        border: 'none',
                        cursor: ftTxPending || ftTransferred || !ftMinted ? 'not-allowed' : 'pointer',
                        fontWeight: 'bold',
                        fontSize: '0.9rem'
                      }}
                    >
                      {ftTxPending === 'transfer' ? 'Transferring...' : ftTransferred ? 'Done' : 'Transfer 30'}
                    </button>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#cbd5e1', fontWeight: 'bold', marginBottom: '0.25rem' }}>Verify totalSupply (100) and your balance (70)</div>
                      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Read from the contract. Total supply stays 100; your balance drops to 70 after the transfer.</div>
                    </div>
                    <button
                      onClick={handleReadFT}
                      style={{
                        padding: '0.5rem 1rem',
                        borderRadius: '0.375rem',
                        background: '#8b5cf6',
                ...
                {(ftReads.totalSupply != null || ftReads.myBalance != null) && (
                  <div style={{
                    marginTop: '1rem',
                    padding: '0.75rem',
                    background: 'rgba(59, 130, 246, 0.1)',
                    ...
                    <div style={{ color: '#93c5fd', fontWeight: 'bold', marginBottom: '0.5rem' }}>Results</div>
                    <div style={{ color: '#cbd5e1' }}>
                      totalSupply: <strong>{ftReads.totalSupply ?? '—'}</strong>
                      {ftReads.totalSupply === '100' && <span style={{ color: '#10b981', marginLeft: '0.25rem' }}>✓</span>}
                      {ftReads.totalSupply != null && ftReads.totalSupply !== '100' && (
                        <span style={{ color: '#fbbf24', marginLeft: '0.25rem', fontSize: '0.85rem' }}>(expected 100)</span>
                      )}
                    </div>
                    <div style={{ color: '#cbd5e1', marginTop: '0.25rem' }}>
                      Your balance: <strong>{ftReads.myBalance ?? '—'}</strong>
                      {ftReads.myBalance === '70' && <span style={{ color: '#10b981', marginLeft: '0.25rem' }}>✓</span>}
                      {ftReads.myBalance != null && ftReads.myBalance !== '70' && ftTransferred && (
                        <span style={{ color: '#fbbf24', marginLeft: '0.25rem', fontSize: '0.85rem' }}>(expected 70 after transfer)</span>
                      )}
                    </div>
                  </div>
                )}
        ...
        <h4 style={{ color: '#8b5cf6', marginTop: '1.5rem', marginBottom: '0.75rem' }}>2. Deploy NFT (Non-Fungible Token)</h4>
        {showDeploy && (
          <>
            <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: '1rem' }}>
              Deploy a minimal NFT (SimpleNFT) from this page, or use the CLI for Event Tickets (Option B above).
            </p>
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1rem' }}>
              <button onClick={handleDeployNFT} disabled={nftDeploying} style={{
                padding: '0.75rem 1.5rem', background: '#8b5cf6', color: '#fff', border: 'none', borderRadius: '0.375rem',
                fontWeight: 'bold', cursor: nftDeploying ? 'disabled' : 'pointer'
              }}>
                {nftDeploying ? 'Deploying...' : 'Deploy NFT (Browser)'}
              </button>
            </div>
            {nftDeployError && <div style={{ color: '#ef4444', marginBottom: '1rem', fontSize: '0.9rem' }}>{nftDeployError}</div>}
            {nftError && <div style={{ color: '#ef4444', marginBottom: '1rem', fontSize: '0.9rem' }}>{nftError}</div>}
            {nftAddress && (
              <div style={{
                background: '#0f172a',
                padding: '1.5rem',
                ...
                <h4 style={{ color: '#8b5cf6', marginTop: 0, marginBottom: '0.5rem' }}>
                  Guided Exercise: NFT in Action
                </h4>
                <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: '1rem' }}>
                  Follow the steps to see how mint, transfer, and ownerOf work. Each token has a unique ID.
                </p>
                <div style={{ color: '#64748b', fontSize: '0.8rem', marginBottom: '1rem' }}>
                  NFT deployed: <code style={{ fontSize: '0.85rem' }}>{nftAddress}</code>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.9rem', color: '#cbd5e1' }}>
                    <span>#1</span>
                    <span style={{ textAlign: 'right', color: '#8b5cf6', fontSize: '0.8rem' }}>
                      {nftReads.ownerOf1 ? (sameAddress(nftReads.ownerOf1, wallet?.address) ? 'You' : sameAddress(nftReads.ownerOf1, ACCOUNT_1) ? 'Account 1' : `${String(nftReads.ownerOf1).slice(0, 10)}...`) : '—'}
                    </span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.9rem', color: '#cbd5e1' }}>
                    <span>#2</span>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.9rem', color: '#cbd5e1', marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid #334155' }}>
                    ...
                    <span style={{ textAlign: 'right', color: '#3b82f6' }}>{nftReads.myBalance ?? '—'}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.9rem', color: '#cbd5e1' }}>
                    <span>Account 1 balance</span>
                    <span style={{ textAlign: 'right', color: '#10b981' }}>{nftReads.account1Balance ?? '—'}</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{
                    ...
                    gap: '0.75rem',
                    padding: '0.75rem',
                    background: nftMinted ? 'rgba(139, 92, 246, 0.1)' : 'rgba(71, 85, 105, 0.2)',
                    borderRadius: '0.375rem',
                    border: `1px solid ${nftMinted ? '#8b5cf6' : '#334155'}`
                  }}>
                    <span style={{
                      width: '28px',
                      height: '28px',
                      borderRadius: '50%',
                      background: nftMinted ? '#8b5cf6' : '#334155',
                      color: '#fff',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#cbd5e1', fontWeight: 'bold', marginBottom: '0.25rem' }}>Mint 3 tickets to yourself</div>
                      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Creates tokens #1, #2, #3. Each has a unique ID.</div>
                    </div>
                    <button
                      onClick={handleMintNFT}
                      disabled={!!nftTxPending || nftMinted}
                      style={{
                        padding: '0.5rem 1rem',
                        borderRadius: '0.375rem',
                        background: nftMinted ? '#334155' : '#8b5cf6',
                        color: '#fff',
                        border: 'none',
                        cursor: nftTxPending || nftMinted ? 'not-allowed' : 'pointer',
                        fontWeight: 'bold',
                        fontSize: '0.9rem'
                      }}
                    >
                      {nftTxPending === 'mint' ? 'Minting...' : nftMinted ? 'Done' : 'Mint 3'}
                    </button>
                  </div>
                  ...
                  <div style={{
                    ...
                    gap: '0.75rem',
                    padding: '0.75rem',
                    background: nftTransferred ? 'rgba(139, 92, 246, 0.1)' : 'rgba(71, 85, 105, 0.2)',
                    borderRadius: '0.375rem',
                    border: `1px solid ${nftTransferred ? '#8b5cf6' : '#334155'}`
                  }}>
                    <span style={{
                      width: '28px',
                      height: '28px',
                      borderRadius: '50%',
                      background: nftTransferred ? '#8b5cf6' : '#334155',
                      color: '#fff',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#cbd5e1', fontWeight: 'bold', marginBottom: '0.25rem' }}>Transfer ticket #1 to Account 1</div>
                      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Move token #1 to {ACCOUNT_1.slice(0, 10)}...</div>
                    </div>
                    <button
                      onClick={handleTransferNFT}
                      disabled={!!nftTxPending || nftTransferred || !nftMinted}
                      style={{
                        padding: '0.5rem 1rem',
                        borderRadius: '0.375rem',
                        background: nftTransferred ? '#334155' : !nftMinted ? '#475569' : '#8b5cf6',
                        color: '#fff',
                        border: 'none',
                        cursor: nftTxPending || nftTransferred || !nftMinted ? 'not-allowed' : 'pointer',
                        fontWeight: 'bold',
                        fontSize: '0.9rem'
                      }}
                    >
                      {nftTxPending === 'transfer' ? 'Transferring...' : nftTransferred ? 'Done' : 'Transfer #1'}
                    </button>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#cbd5e1', fontWeight: 'bold', marginBottom: '0.25rem' }}>Verify ownerOf(1) is Account 1</div>
                      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Read from the contract. After transfer, token #1 should belong to Account 1.</div>
                    </div>
                    <button
                      onClick={handleReadNFT}
                      style={{
                        padding: '0.5rem 1rem',
                        borderRadius: '0.375rem',
                        background: '#10b981',
                ...
                {/* Expected-result feedback */}
                {(nftReads.ownerOf1 != null || nftReads.myBalance != null) && (
                  <div style={{
                    marginTop: '1rem',
                    padding: '0.75rem',
                    background: 'rgba(59, 130, 246, 0.1)',
                    ...
                    <div style={{ color: '#93c5fd', fontWeight: 'bold', marginBottom: '0.5rem' }}>Results</div>
                    <div style={{ color: '#cbd5e1' }}>
                      ownerOf(1): <strong>{nftReads.ownerOf1 ? (sameAddress(nftReads.ownerOf1, ACCOUNT_1) ? 'Account 1' : String(nftReads.ownerOf1).slice(0, 12) + '...') : '—'}</strong>
                      {nftTransferred && sameAddress(nftReads.ownerOf1, ACCOUNT_1) && <span style={{ color: '#10b981', marginLeft: '0.25rem' }}>✓</span>}
                      {nftTransferred && nftReads.ownerOf1 && !sameAddress(nftReads.ownerOf1, ACCOUNT_1) && (
                        <span style={{ color: '#fbbf24', marginLeft: '0.25rem', fontSize: '0.85rem' }}>(expected Account 1)</span>
                      )}
                    </div>
                    <div style={{ color: '#cbd5e1', marginTop: '0.25rem' }}>
                      Your balance: <strong>{nftReads.myBalance ?? '—'}</strong>
                      {nftTransferred && nftReads.myBalance === '2' && <span style={{ color: '#10b981', marginLeft: '0.25rem' }}>✓</span>}
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* Fallback: simulated visualizers - show first when using simulators */}
        {showFallback && (
          <>
            ...
            {/* Expandable CLI instructions - collapsed by default to reduce scroll */}
            <div style={{
              marginTop: '2rem',
              border: '1px solid #334155',
              borderRadius: '0.5rem',
              ...
              >
                Advanced: Deploy via CLI
                <span style={{ fontSize: '1rem' }}>{cliExpanded ? '▼' : '▶'}</span>
              </button>
              {cliExpanded && (
                <div style={{
                  padding: '1rem',
                  ...
                  <div style={{ color: '#93c5fd', fontWeight: 'bold', marginBottom: '0.5rem' }}>How to deploy</div>
                  <p style={{ color: '#cbd5e1', fontSize: '0.9rem', margin: '0 0 0.5rem 0' }}>
                    <strong>Option A (Browser):</strong> Set up wallet + ETH + RPC in the Live tab, then return here.
                  </p>
                  <p style={{ color: '#cbd5e1', fontSize: '0.9rem', margin: '0 0 0.75rem 0' }}>
                    <strong>Option B (CLI):</strong> Open a terminal. Ensure the blockchain is running (<code style={{ fontSize: '0.85rem' }}>npm run chain</code> or instructor node). Run <code style={{ background: 'rgba(0,0,0,0.3)', padding: '0.15rem 0.4rem', borderRadius: '0.25rem', fontSize: '0.85rem' }}>node scripts/cli-labs/standalone/5-contract-builder.js</code>. Choose 1 (Build New Contract), then pick <strong>Simple Fungible Token</strong>, <strong>Simple NFT</strong>, or <strong>Event Tickets</strong>.
                  </p>
                  <div style={{ color: '#10b981', fontWeight: 'bold', marginBottom: '0.25rem', fontSize: '0.9rem' }}>1. Deploy FT</div>
                  <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: '0 0 0.5rem 0' }}>
                    Select <strong>Simple Fungible Token (Token Concepts)</strong> from the template list.
                  </p>
                  <div style={{ color: '#8b5cf6', fontWeight: 'bold', marginBottom: '0.25rem', fontSize: '0.9rem' }}>2. Deploy NFT</div>
                  <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>
                    Select <strong>Event Tickets (Token Concepts - NFT)</strong>. Deploy, buy tickets, transfer one, check <code>ownerOf</code>.
                  </p>
                </div>
              )}
            </div>
